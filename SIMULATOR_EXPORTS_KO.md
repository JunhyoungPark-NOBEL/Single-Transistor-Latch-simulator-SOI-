...
두 형식 모두 L·W·Tsi·EOT·Tbox·Nbody, VG·VBG, 광전류·보정값을 고정한 **준정적 동작 모델**입니다. 웹 엔진에서 계산한 안정 branch 전류를 log(I) 선형 보간하고, 같은 VLU/VLD를 사용합니다. 단자는 D, S입니다. 치수와 바이어스는 파일 메타데이터에 기록되며, 보정이나 고정 조건을 바꿨다면 다시 계산하고 내보내야 합니다. 상용 프로그램 안에서 치수만 바꿔 재계산하는 물리 모델은 아닙니다.
