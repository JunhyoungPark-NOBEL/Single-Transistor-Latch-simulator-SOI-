"""Current-carrying quasi-neutral high-injection candidate.
...
def components(u,r,p,na,vbi,rg,fg,table):
    ...
    beta,tau,tj,rc,lg,ta,naa,la=p
    eps=11.7*EPS0/100
    wd=np.sqrt(2*eps*(vbi+r)/(Q*na))
    # max(...,0) is a FORMAL post-flat-band continuation for diagnostics.
    # States u>=Vbi fail the abrupt-depletion validity test and must be reported
    # as unsupported; finite numerical output does not validate those states.
    ws=np.sqrt(2*eps*max(vbi-u,0.)/(Q*na))
    length=LENGTH_M*100-wd-ws
    if length<=1e-7:return np.full(17,np.nan)
    ...
    js,lb,qa,iterations=transport_solve(delta/na,mult,(bbj+gidl)/scale,length*length/(DN*tau),1/3,64)
    if not np.isfinite(js):return np.full(17,np.nan)
    ...
    vd=u+r+(rc+racc)*drain
    qpair=Q*AREA_CM2*length*avg
    qaccess=Q*(WIDTH_M*100*ta*1e-7)*(la*1e-7)*avg
    charge=COX_F*u+qpair+qaccess
    return np.array([vd,drain,net,seed,emitter,bulk,diff,junction,bbj,gidl,delta/na,length,racc,charge,ws,wd,ch])

@njit(cache=True)
def curve_grid(p,na,vbi,rg,fg,table,ug):
    out=np.empty((len(ug),20));count=0
    eps=11.7*EPS0/100
    for u in ug:
        if u==0:
            z=components(0.,0.,p,na,vbi,rg,fg,table)
            out[count,:17]=z;out[count,17]=0.;out[count,18]=0.
        ...
        out[count,:17]=z;out[count,17]=u;out[count,18]=r
...
class FastModel:
    ...
    def double_curve(self,p,points=201):
    ...
    def validity_flags(self,output):
        return dict(numerically_missing=int(np.sum(~np.isfinite(output[:,1]))),
                    source_flatband_or_beyond=int(np.sum(output[:,2]>=self.vbi)),
