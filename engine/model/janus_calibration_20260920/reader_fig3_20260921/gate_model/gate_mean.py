...
def components(u,r,p,na,vbi,rg,fg,table):
    ...
    n=1.7786684648788609;ov=vg-(-.49032524444873615);pp=ov/n
    ...
    js,lb,qa,hole_drop_over_VT,iterations=srh_solve(delta/na,mult,(bbj+gidl+ii_ch)/scale,length*length/(DN*tau),1/3,64,tau_ratio,(1+tau_ratio)*NI_CM3/na)
    ...
    volume=WIDTH_M*100*5e-7*depth
    gidl=Q*volume*BB_A*eg**2.5*np.exp(-BB_B/max(eg,1.))*balance
    drain=seed+ii+bbj+gidl+ch+ii_ch
    net=ii+ii_ch+bbj+gidl-diff-bulk-junction
