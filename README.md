...
**Deterministic / Stochastic**을 고르고, 왼쪽 파라미터 카드에서 조건을 정한 뒤 실행합니다. 모든 그룹·패널의
**상세** 버튼은 같은 화면에 작은 창을 띄워 코드와 정확히 일치하는 수식·변수표·가정을 보여 줍니다.

- **소자 (Device)** — Deterministic: 정상상태 I–V branch(HRS/불안정/LRS, fold 전압 V_LU·V_LD), 전류 성분, 고정
...
- **회로 (Circuit)** — 바디 전하 Q_B를 상태변수로 갖는 STL 소자를 넣은 MNA 과도해석(후진 오일러/사다리꼴 +
  ...
  테스트 벤치: 부하선 스윕, 펄스 열, p-bit, 결합 쌍.
- **검증 (Validation)** — `engine/docs/VALIDATION.md` 기준 수치 재현(빠른/전체 검사)과 측정 기록 비교(기준 측정
  기록 I–V, 광조사 8개 조건, V_LU·σ_LU의 V_G 의존성).
- **물리 모델 (Physics)** — 전체 수식 문서(목차·검색).

만든 곳: KAIST 전기및전자공학부 · NOBEL 연구실 (지도교수 최양규) · 개발 박준형. 화면 오른쪽 위 모드 막대의
크레딧을 누르면 소개 창(버전 포함)이 열립니다.
...
unpublished — for research use only. Pick **Deterministic / Stochastic** at the top, set the grouped parameters
in the sidebar and run. Every group and panel has a **Details** button that opens a compact floating window with
the exact code-level equations, variable tables and assumptions.

Credits: NOBEL Lab · Prof. Yang-Kyu Choi · School of Electrical Engineering, KAIST · Developed by Junhyoung Park.
The credits at the right end of the mode strip open an About card (with the app/engine version).
