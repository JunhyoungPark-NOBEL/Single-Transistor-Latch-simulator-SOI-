...
import { geometryFromDevice, stochOf, type LibDevice } from "./library";
...
/** "L 500 · T_Si 50 nm · V_G −1.8 V": what tells saved calibrations apart at a glance. */
function SlotMeta({ d }: { d: LibDevice }) {
  const g = geometryFromDevice(d.device);
  const n = (v: number) => (Number.isInteger(v) ? String(v) : v.toFixed(1));
  return <span className="slot-meta">L {n(g.Lg_nm)} · T<sub>Si</sub> {n(g.Tsi_nm)} nm · V<sub>G</sub> {signed(d.device.vg, 2).replace(/\.?0+$/, "")} V</span>;
}
...
export function DeviceShelf() {
  ...
  const load = (d: LibDevice) => { loadDeviceIntoParams(d); setMessage(t("schematic.dev.loadedToast", { name: d.name })); };
  ...
  return <aside className="device-shelf" aria-label={L("사용자 소자", "My devices")} data-testid="device-shelf">
    <div className="shelf-head"><h2>{L("사용자 소자", "My devices")}</h2><span className="shelf-count">{devices.length} / {MAX_USER_DEVICES}</span></div>
    <button className="btn primary shelf-save" type="button" disabled={full} onClick={() => setSaving(true)} data-testid="dev-save-open"><span aria-hidden>＋</span>{L("현재 소자 저장", "Save current device")}<span className="shelf-save-count">({devices.length}/{MAX_USER_DEVICES})</span></button>
    ...
    <div className="shelf-slots">
      {Array.from({ length: Math.max(MAX_USER_DEVICES, devices.length) }, (_, i) => {
        ...
        return d ? <article key={d.id} className="device-slot filled" data-testid={`device-slot-${i + 1}`}>
          ...
          <SlotMeta d={d} />
          <div className="slot-actions"><button type="button" className="link-btn" onClick={() => load(d)}>{L("불러오기", "Load")}</button><button type="button" className="link-btn" onClick={() => setReplace(replace === d.id ? null : d.id)}>{L("업데이트", "Update")}</button></div>
    ...
    <div className="shelf-export"><ModelExportButton device={device} sweep={sweep} name={devices.find((d) => deepEqual(d.device, device))?.name} /></div>
