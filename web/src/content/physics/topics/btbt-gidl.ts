...
const topic: PhysicsTopic = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: {
        ko: String.raw`Simple Model은 논문 Table I의 $I_{\mathrm{BTBT}}=q\int A E^{2.5}\exp(-B/E)\,dv$ ($A=4\times10^{14}$, $B=19$ MV/cm)를 논문 참고 스크립트처럼 "한 전계에서의 Kane 생성률 × 체적"으로 평가한다. 접합 BTBT는 급격 드레인 접합의 최대 전계 $E_{\mathrm j}=2(V_{\mathrm{bi}}+r)/W_{\mathrm d}$와 체적 $W T_{\mathrm{Si}} W_{\mathrm d}$를 쓰고, GIDL은 수직 전계 $E_{\mathrm g}=(r-V_{\mathrm G}+1.2-1.12)/(3\,\mathrm{EOT})$와 체적 $s_{\mathrm{GIDL}}\,W\cdot5\,\mathrm{nm}\cdot W_{\mathrm t}$ (기본 $s_{\mathrm{GIDL}}=100$, 스크립트의 유효 체적)를 쓴다. $V_{\mathrm G}$가 평탄대 $V_{\mathrm{FB}}$ 아래로 내려가면 바디 바이어스는 더 변하지 않지만 $E_{\mathrm g}$는 계속 커지므로 GIDL이 정공 생성을 지배하게 되고 $V_{\mathrm{LU}}$가 다시 내려간다(논문 Fig. 5(a), (b)). 위의 국소 전계 적분과 전계 테이블은 Detailed Model의 정의이다.`,
        en: String.raw`The Simple Model evaluates the paper's Table I integral $I_{\mathrm{BTBT}}=q\int A E^{2.5}\exp(-B/E)\,dv$ ($A=4\times10^{14}$, $B=19$ MV/cm) as "Kane rate at one field × volume", as the paper's reference script does. Junction BTBT uses the peak field of the abrupt drain junction, $E_{\mathrm j}=2(V_{\mathrm{bi}}+r)/W_{\mathrm d}$, over the volume $W T_{\mathrm{Si}} W_{\mathrm d}$; GIDL uses the vertical field $E_{\mathrm g}=(r-V_{\mathrm G}+1.2-1.12)/(3\,\mathrm{EOT})$ over $s_{\mathrm{GIDL}}\,W\cdot5\,\mathrm{nm}\cdot W_{\mathrm t}$ (default $s_{\mathrm{GIDL}}=100$, the script's effective volume). Once $V_{\mathrm G}$ drops below the flat band $V_{\mathrm{FB}}$ the body bias no longer changes while $E_{\mathrm g}$ keeps growing, so GIDL takes over hole generation and $V_{\mathrm{LU}}$ comes back down (paper Fig. 5(a), (b)). The local field integral and field tables above belong to the Detailed Model.`,
      },
    },
  ],
