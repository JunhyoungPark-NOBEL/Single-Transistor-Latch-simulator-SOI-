...
const topic: PhysicsTopic = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: {
        ko: "`classify()`: $V_D$ 차분의 부호 변화로 극대·극소를 찾고, 첫 극대 $i$와 그 뒤 마지막 극소 $j$를 fold로 쓴다. 각 fold는 $(k-1, k, k+1)$ 세 점에서 $V_D(u - u_k)$ 2차 다항식(`np.polyfit`)의 꼭짓점 값으로 보정한다(식은 `charge-balance`). 8점 미만·극값 없음이면 `None`(웹: `latch = false`). 웹은 fold의 세 행이 $u$에서 이어지지 않으면(간격 > 0.05 V; 정상 fold는 격자 한 칸) 그 fold를 추적 불가로 버리고 `latch = false`와 경고를 낸다: $V_G \\gtrsim 0$ V에서는 $10^{-40} < u < 0.84$ V에 정상 상태가 없어 `classify()`가 간격 너머 첫 행을 극대로 잡고 포물선을 간격에 걸쳐 맞춘다(`deterministic.classify_checked`, 확률 fold 노드 포함). `sample_root()`는 고정 $V_D$에서 $u$를 13회 이분해 한 상태를 찾는 보조 함수다.",
        en: "`classify()` finds maxima/minima from sign changes of the $V_D$ differences and takes the first maximum $i$ and the last minimum $j$ after it. Each fold is refined by the vertex of a quadratic `np.polyfit` of $V_D(u - u_k)$ through $(k-1, k, k+1)$ (formula in `charge-balance`). Fewer than 8 points or missing extrema give `None` (web: `latch = false`). The web server also rejects a fold whose three rows are not contiguous in $u$ (gap > 0.05 V; a real fold spans one grid step) as not traceable, with `latch = false` and a warning: for $V_G \\gtrsim 0$ V there is no steady state for $10^{-40} < u < 0.84$ V, so `classify()` takes the first row after the gap as the maximum and fits its parabola across the gap (`deterministic.classify_checked`, stochastic fold nodes included). `sample_root()` is a helper that bisects $u$ 13 times to find the state at a fixed $V_D$.",
      },
    },
    {
