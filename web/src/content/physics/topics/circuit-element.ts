...
const topic: PhysicsTopic = {
  ...
  sections: [
    ...
    },
    {
      ...
      notes: [
        L(
          "초기 동작점: $u=0$(빈 body)으로 고정해 풀고(커패시터 개방), 의사과도 BE($h$: $10^{-12}$ s에서 ×3, 최대 $10^8$ s)로 빈 body에서 도달하는 저전류 상태를 찾는다(쌍안정 구간이면 HRS). 초기 latch 상태는 body의 가지로 정한다($u \\ge u_j$, 아래 latch 검출).",
          "Initial operating point: solve with $u=0$ (empty body, capacitors open), then pseudo-transient BE ($h$ from $10^{-12}$ s, ×3 per step, up to $10^8$ s) to the low-current state reachable from an empty body (the HRS inside the hysteresis window). The initial latch state is the body's branch ($u \\ge u_j$, see latch detection below).",
        ),
      ],
    },
    {
      ...
      equations: [
        ...
        },
        {
          ...
          note: L(
            "$z_{\\max}=12$ (`noise_z_max`)는 정확한 compound-jump hazard로 보정했다: 대역 밖 탈출률 ≲ $10^{-3}$ s⁻¹. 대역 밖은 drift만 적분한다(정상 요동 몇 mV는 계산하지 않음). fold 너머 쪽에는 경계가 없다: latch 안 된 셀이 $V_{LU}$ 위에 있으면 fold 이후 통과 중이고, 그 잡음이 빠른 램프의 $V_{LU}$와 fold 위 펄스의 지연을 정한다. 대역 진입 $4\\tau_{\\mathrm{rel}}$ 전부터 잡음을 켜므로(look-ahead) 빠른 램프·에지에서도 정상 요동이 미리 쌓인다. 확장 $e$는 GIDL 작용점의 국소 상태 fold 이동 4σ; 다른 작용점이나 $\\sigma_E>2$ mV는 대역 제한 없음; latch 창이 없는 소자는 국소 상태가 없으면 잡음 없이(drift만) 적분한다. 예(국소 상태 없음): 논문 −2 V 암조건 HRS 3.59 V 이상, LRS 2.82 V 이하; photo 프리셋 HRS 3.70 V 이상; −1.8 V·2.63 pA HRS 2.93 V 이상. 이 대역 덕분에 0.4 V/s 논문 소자도 사건 수준으로 계산 가능하다(사이클당 약 2×10⁵ 스텝).",
            "$z_{\\max}=12$ (`noise_z_max`) was calibrated against the exact compound-jump hazard: escape rate ≲ $10^{-3}$ s⁻¹ outside the bands. Outside the bands only the drift is integrated (the stationary fluctuation of a few mV is not simulated). There is no edge beyond the fold: an unlatched cell above $V_{LU}$ is in the post-fold passage, whose noise sets $V_{LU}$ on fast ramps and the delay of supra-fold pulses. The noise is switched on $4\\tau_{\\mathrm{rel}}$ before the drive enters a band (look-ahead), so the stationary fluctuation is built up on fast ramps and edges too. The widening $e$ is 4σ of the local-state fold shifts for the GIDL action point; other action points or $\\sigma_E>2$ mV get no band restriction; a device without latch window is integrated drift-only unless local states are on. Examples (no local states): paper −2 V dark HRS from 3.59 V, LRS up to 2.82 V; photo preset HRS from 3.70 V; −1.8 V, 2.63 pA HRS from 2.93 V. The bands make event-level runs of the paper device at 0.4 V/s affordable (about 2×10⁵ steps per cycle).",
          ),
    ...
    },
    {
      ...
      variables: [
        ...
        { symbol: r`\mathrm{LTE}_u`, name: L("u 국소 절단오차 (램프 지연을 약 1 %까지 수렴)", "local truncation error in u (converges the ramp lag to about 1 %)"), value: "0.03", unit: "mV", code: "CF_LTEU" },
        ...
        { symbol: r`z_{\max}`, name: L("잡음 대역 장벽 한계 (fold 너머 쪽은 경계 없음)", "noise-band barrier limit (no edge beyond the fold)"), value: "12", code: "solver.noise_z_max" },
        { symbol: r`I_{th}`, name: L("latch-up 시각 문턱", "latch-up timing threshold"), value: "10", unit: "nA", code: "detect.i_threshold_A" },
        { symbol: r`I_{th}/\mathrm{hyst}`, name: L("latch-down 시각 문턱 (hysteresis 10)", "latch-down timing threshold (hysteresis 10)"), value: "1", unit: "nA", code: "detect.hysteresis" },
      ],
      notes: [
        L(
          "10 nA latch-up 문턱은 FPT 격자의 흡수 경계와 같다(→ first-passage). LD fold의 LRS 전류가 약 16 nA($V_G=-2$ V: 16.7 nA)뿐이라 latch-down 시각은 더 낮은 1 nA에서 잰다. 문턱은 시각만 정하고 latch 상태와 잡음 계층은 바꾸지 않는다; 문턱이 fold의 가지 전류(HRS 약 15 pA, LRS 약 17 nA) 사이에 있지 않으면 경고.",
          "The 10 nA latch-up threshold equals the absorbing boundary of the FPT lattice (→ first-passage). The LRS current at the LD fold is only about 16 nA (16.7 nA at $V_G=-2$ V), so latch-down is timed at the lower 1 nA. The thresholds only time the events; they change neither the latch state nor the noise tiers, and a warning is given when a threshold does not lie between the branch currents at its fold (HRS about 15 pA, LRS about 17 nA).",
        ),
        L(
          "실행 가능성: 실행 전에 준정적 가지의 $\\tau_{\\mathrm{rel}}(V_D)$·사건률과 위 스텝 규칙으로 실행당 스텝 수를 추정한다. 잡음에 의한 fold 전 탈출(경험적 hazard)도 반영한다(부하선 약 ±20 %, 확률적 펄스·p-bit는 최대 약 1.6배 과대). $2\\times$`max_steps`(기본 $10^6$)를 넘으면 거부, $0.5\\times$를 넘으면 경고, 요청 전체가 $4\\times10^7$ 스텝을 넘으면 거부. 구현 세부: docs/CIRCUIT_SIMULATOR.md.",
          "Feasibility: before running, the steps per run are estimated from $\\tau_{\\mathrm{rel}}(V_D)$ and the event rate of the quasi-static branches with the step rules above, including the noise-induced escape before the fold (empirical hazard); load lines typically within about ±20 %, probabilistic pulse and p-bit trains up to about 1.6× too high. Above $2\\times$`max_steps` (default $10^6$) the run is refused, above $0.5\\times$ it is warned, and a request above $4\\times10^7$ steps in total is refused. Implementation detail: docs/CIRCUIT_SIMULATOR.md.",
        ),
      ],
    },
    {
      ...
      equations: [
        {
          ...
          note: L("$t^{\\mathrm{top}}_n$: $n$번째 펄스 평탄부의 끝. latch 상태 = body의 가지(위 latch 검출). $N$은 실행이 도달한 펄스 수.", "$t^{\\mathrm{top}}_n$: end of the flat top of pulse $n$. Latched = the body's branch (latch detection above). $N$ counts the pulses the run reached."),
        ...
        },
        {
          ...
          note: L("$b_n$은 비교기 출력이다. 보통 $b_n=1$은 셀이 latch되어 $v_D$가 부하선을 따라 내려간 상태지만, 채널이 켜지면(V_G가 채널 문턱 위) latch 없이도 $v_D$가 내려가 1이 된다 — 그때는 경고하고 $P_{\\mathrm{latched}}$를 따로 보고한다.", "$b_n$ is the comparator output. Normally $b_n=1$ means the cell latched and $v_D$ dropped along the load line; with the channel on ($V_G$ above the channel threshold) $v_D$ drops without a latch, which is warned about, and $P_{\\mathrm{latched}}$ is reported separately."),
