...
// shared job runner (polling, cancel, HTTP 429 retry), demo fallback while the server does not know
// bench "custom" yet, result slot results["schematic"] (so the Run bar and panels show progress).
...
let token = 0;
let demoBackend: ReturnType<typeof createMockBackend> | null = null;
...
export async function runSchematic(opts: { confirmed?: boolean } = {}): Promise<void> {
  ...
  const aborted = () => token !== my || useStore.getState().results[RESULT_KEY]?.status === "cancelled";
  const finish = () => {
  ...
  try {
    let result: CustomCircuitResult;
    try {
      result = await run(backend);
    } catch (e) {
      // the server answers but does not know bench "custom" yet → demo result (clearly marked)
      if (!(e instanceof JobAborted) && e instanceof ApiError && isUnknownBenchError(e.message) && !backend.isMock) {
        demoBackend ??= createMockBackend(0.6);
        backend = demoBackend;
        demoFallback = true;
        result = await run(backend);
      } else throw e;
