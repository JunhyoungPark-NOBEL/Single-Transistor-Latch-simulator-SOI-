...
export type Tab = "device" | "circuit" | "validation" | "physics" | "performance";
...
const isDetailed = (c: Ctx) => c.root.device.model !== "simple";
...
export const GROUPS: GroupDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "loc_carriers", path: ["device", "ext", "loc_carriers"], sym: "c_{\\mathrm{loc}}", label: L("국소 경로 캐리어", "Local-path carriers"), help: L("가설 경로의 캐리어를 선택합니다. 채널을 제외한 가장자리 경로는 현재 지원하지 않습니다.", "Select carriers for this hypothesis. The edge path excluding channel carriers is currently unsupported."), code: "p[24]", type: "select", experimental: true,
        options: [
          ...
          { value: 1, label: L("1 · 벌크", "1 · bulk") },
          {
            ...
            note: L("미지원 · 잘못된 경로 계산을 방지하기 위해 비활성화했습니다.", "Unsupported · disabled to prevent calculation through the wrong path."),
          },
        ],
      },
...
export function groupVisible(g: GroupDef, c: Ctx): boolean {
  if (!g.tabs.includes(c.tab === "validation" || c.tab === "physics" || c.tab === "performance" ? "device" : c.tab)) return false;
