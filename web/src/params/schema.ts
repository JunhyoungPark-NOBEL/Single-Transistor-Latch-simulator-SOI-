...
export const GROUPS: GroupDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "l_gidl", path: ["device", "calib", "l_gidl_nm"], sym: "l_{\\mathrm{GIDL}}", label: L("GIDL 전계 길이", "GIDL field length"), help: L("GIDL 전계를 정하는 유효 길이: E_G = max(u + r − V_G − 0.3 − 1.12 + φ_GIDL, 0) / l_GIDL (BTBT 영역의 크기가 아님)", "Effective length that sets the GIDL field: E_G = max(u + r − V_G − 0.3 − 1.12 + φ_GIDL, 0) / l_GIDL (not the size of the BTBT region)"), code: "p[4]", unit: "nm", min: 0.1, max: 500, slider: "log" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "c_ith", path: ["circuit", "detect", "i_threshold_A"], sym: "I_{\\mathrm{th}}", label: L("스위칭 판정 전류", "Switch-detect current"), help: L("latch 사건 판정 문턱 (I_D가 이 값을 위/아래로 지나면 latch-up/down) — 두 모드 모두 사용", "Threshold used to detect latch events (I_D crossing it up/down = latch-up/down) — used in both modes"), unit: "A", min: 1e-15, max: 1e-2, slider: "log" },
