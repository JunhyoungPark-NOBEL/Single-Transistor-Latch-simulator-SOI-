...
These test-only fixtures were captured from the unchanged Python run_circuit solver with the full PRESETS.paper.device and the request in payload.json. The calibrated Device 1 uses VG = -2 V. Default: Iin=1nA, Cdrain=1pF, Time=15ms, BE, reltol=1e-3, dt_max=Time/3000; event detection1e-8A / hysteresis10. The physical waveform and event list are not synthesized by the UI tests.
